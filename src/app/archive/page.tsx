...
export default async function ArchivePage({
  ...
  try {
    ...
    if (status === 422) {
      ...
      backendError = detailText ?? "Invalid search filters. Please check your date range.";
      usingBackend = true;
      results = [];
      totalResults = 0;
    } else {
  ...
  const resultNoun = usingBackend ? (view === "pages" ? "page" : "snapshot") : "snapshot";
  const formattedTotalResults = new Intl.NumberFormat(undefined).format(totalResults);
      ...
      {sourceSummaries.length > 0 && (
        <section className="ha-home-panel-gradient ha-home-panel-gradient-compact mb-4 space-y-3">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <div>
              <h2 className="text-sm font-semibold text-slate-900">Browse archived sites</h2>
            </div>
            <Link
                  ...
                  >
                    ...
                    <div className="pt-2.5 pb-1 sm:pt-3 sm:pb-1">
                      ...
                      <p className="text-ha-muted mt-1 text-xs whitespace-nowrap">
                        {summary.recordCount.toLocaleString()} snapshot
                        {summary.recordCount === 1 ? "" : "s"} · latest{" "}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="inline-flex items-center gap-1">
                    ...
                    >
                      ...
                      <option value="snapshots">All snapshots</option>
                    </select>
                  </div>
            ...
            {totalResults === 0 ? (
              <div className="ha-card ha-home-panel p-4 sm:p-5">
                <p className="text-ha-muted text-sm">
                  No records match the current filters. Try removing some filters, using broader
                  keywords, or{" "}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {usingBackend && (
                  <span className="text-ha-muted text-[11px]">
                    ...
                    {Math.min(effectivePage * paginationSize, totalResults)} of {totalResults}
                  </span>
                )}
              </div>
