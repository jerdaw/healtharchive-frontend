...
export function ChangingPhrase({ text, className }: ChangingPhraseProps) {
    ...
    useEffect(() => {
        ...
        const indices = finalChars.map((char, index) =>
            char === " " ? null : index,
        );
        const nonSpaceIndices = indices.filter(
        ...
        <span className={className}>
            <span aria-hidden="true" className="inline-block whitespace-nowrap">
                {displayedChars.map((char, index) => (
                    <span
                        key={`${char}-${index}`}
                        className="inline-block align-baseline transition-transform transition-opacity duration-150 ease-out"
                    >
                        {char === " " ? "\u00A0" : char}
                    </span>
                ))}
